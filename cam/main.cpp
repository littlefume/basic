	...
	ISampleGrabberCB : IUnknown
{
	...
	virtual STDMETHODIMP SampleCB(double Time, IMediaSample *pSample)
	{
		return E_NOTIMPL;
	}
...
int main(int argc, char **argv)
{
	...
	// Set CallBack
	hr = pSampleGrabber->SetCallback((DexterLib::ISampleGrabberCB*)&SCB, 1);
	if (hr != S_OK) exit_message("Could not sample grabber call back", 1);
		...
		(mt.pbFormat != NULL))
	{
		...
		// required size
		int nChars = MultiByteToWideChar(CP_ACP, 0, filename, -1, NULL, 0);
		// allocate it
		pwcsName = new WCHAR[nChars];
		MultiByteToWideChar(CP_ACP, 0, filename, -1, (LPWSTR)pwcsName, nChars);

		// Open output file
		HANDLE hf = CreateFile(pwcsName, GENERIC_WRITE,
